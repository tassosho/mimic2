  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    try:
      ...
      if args.restore_step:
        ...
        log(
            f'Resuming from checkpoint: {restore_path} at commit: {commit}',
            slack=True,
        )
      else:
        log(f'Starting new training run at commit: {commit}', slack=True)
      ...
      while not coord.should_stop():
        ...
        if step % args.checkpoint_interval == 0:
          ...
          log(f'Input: {sequence_to_text(input_seq)}')
